...
shell-smoke-test: ## 🧪 Run shell script syntax checks (bash -n)
	...
	@./tests/shell-smoke-test.sh
	@echo "$(GREEN)✅ Shell smoke tests passed!$(RESET)"
...
python-lint: ## 🐍 Lint Python files with flake8
	...
	@echo "$(DIM)  • Running mypy type checks...$(RESET)"
	@if command -v mypy >/dev/null 2>&1; then \
	  mypy --ignore-missing-imports core/*.py scripts/*.py modular/*.py misc/*.py || (echo "$(RED)❌ mypy type checks failed!$(RESET)" && exit 1); \
	elif python3 -m mypy --version >/dev/null 2>&1; then \
	  python3 -m mypy --ignore-missing-imports core/*.py scripts/*.py modular/*.py misc/*.py || (echo "$(RED)❌ mypy type checks failed!$(RESET)" && exit 1); \
	else \
...
bash-lint: ## 📜 Lint Bash scripts with shellcheck and shfmt
	...
	@find . -name '*.sh' -type f -not -path './venv/*' -not -path './generated-networkpolicies/*' -not -path './complianceremediations/*' -not -path './test-runs/*' -not -path './testing/*' -not -path './docs/vendor/*' | xargs shellcheck -e SC1091,SC2034,SC2001,SC2028,SC2129,SC2155,SC2317,SC2329 || (echo "$(RED)❌ shellcheck failed!$(RESET)" && exit 1)
