...
def parse_machineconfig_files(src_dir):
    ...
    combo_map = defaultdict(list)  # path -> list of (source_file, decoded_lines)
...
def write_combo_yaml(path, sources, out_dir):
    """Write a combined MachineConfig YAML for a given file path and list of sources."""
    all_lines = set()
...
def move_originals_to_combo(combo_map, src_dir, combo_dir):
    """Move original YAMLs that were combined to the combo subfolder."""
    moved = set()
